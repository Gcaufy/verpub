...
const { updateInfoMap } = require('./util/updateInfoMap');
...
function tryLoad(file) {
  ...
  if (ext === '' || ext === '.config') {
    return tryLoad(p + '.js') || tryLoad(p + '.json');
  }
  if (fs.existsSync(p)) {
    return require(p);
  }
...
exports = module.exports = class VerPub {
  constructor(opt = {}) {
    ...
    let configOpt = tryLoad('verpub.config');
    if (!configOpt) {
    ...
    return this.executer
      ...
      .then(() => {
        return this.executer.githash().then(hash => {
          ...
          const infoMap = this.opt.updateInfo || ['version', '_id', '_commitid'];
          const version = params.version;
          const updateInfo = updateInfoMap({
            infoMap,
            id,
            version,
            hash
          ...
          return writeJsonFile(targetPkgJsonFile, Object.assign({}, pkg, updateInfo), {
            detectIndent: true,
            normalize: true
          }).then(() => {
