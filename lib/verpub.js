  ...
  replaceRst.forEach(rst => {
    ...
    if (expected[p] !== undefined) {
      if (expected[p] === rst.numMatches && expected[p] === rst.numReplacements) {
        logger.info(`In ${chalk.gray(rst.file)} ${chalk.red(rst.numMatches)} matched and ${chalk.green(rst.numReplacements)} replaced as expected`);
      } else {
        throw new Error(`In ${chalk.gray(rst.file)} ${chalk.red(rst.numMatches)} matched and ${chalk.green(rst.numReplacements)} replaced, but expected ${chalk.green(expected[p])}`);
      }
    } else {
      if (rst.hasChanged) {
        logger.info(`In ${chalk.gray(rst.file)} ${chalk.red(rst.numMatches)} matched and ${chalk.green(rst.numReplacements)} replaced`);
      } else {
        logger.warn(`In ${chalk.gray(rst.file)} ${chalk.red(rst.numMatches)} matched and ${chalk.green(rst.numReplacements)} replaced`);
      }
    }
  })
}
    ...
    return this.executer.isGitWorkTreeClean().catch(e => {
      ...
      return this.executer.githash().then(hash => {
        ...
        if (this.opt.files && this.opt.files.length) {
          replaceFiles(this.logger, this.opt.files, new RegExp(pkg.version.replace(/\./g, '\\.'), 'g'), params.version);
        }
